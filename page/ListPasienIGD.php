...
<div class="table-responsive">
	<table width="100%" cellpadding="0px" cellspacing="0" id="myTable1" style="font-size:10px" border="1px">
		...
		<tr>
			...
			<th width="120" align="left">POLIKLINIK</th>
		...
		<?
		$i = 0;
		$sql = "SELECT * FROM registerigd
where id>0 " . $search . " ORDER BY TanggalPulang ASC ";
		$pager = new PS_Pagination($connect, $sql, 10, 5,  "&nama=" . $nama . "&ruangan=" . $ruangan . "&urut=" . $urut . "&nomr=" . $nomr . "&per=" . $per . "&status=" . $status, "index.php?link=ListPasienIGD&");
		...
		while ($data = mysql_fetch_array($rs)) {
			...
			<tr title="<?= $data['NamaPasien'] ?> (<?= $data['NamaPasien'] ?>)">
				...
				<td align="center"><?= $data['NamaAsuransi'] ?> </td>
				<td align="center"><?= $data['cek'] ?></td>
				<td align="center">
					<a href='index.php?link=PilihPasienRajal&id_admission=<?= $data['IdRegisterKunjungan'] ?>' class="btn btn-sm btn-outline-success">&#187;</a>
				</td>
			</tr>
		<?	}
		?>
